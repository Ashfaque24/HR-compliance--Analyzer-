...
export default function AssessmentFlow() {
  ...
  const surveySections = useMemo(() => {
    ...
    questions.forEach((q) => {
      const sectionId = q.section_id;
      const sectionTitle =
        q.Section?.title || q.section_name || `Section ${sectionId}`;
  ...
  const isSelected = (qIdx, optId) =>
    responses[currentSectionIdx]?.[qIdx] === optId;
  ...
  const handleSubmit = async () => {
    ...
    try {
      const result = await dispatch(
        submitAssessmentAnswers({ session_uuid, answers })
      ).unwrap();
  ...
  // Pick background color for current section (cycle if more sections than colors)
  const currentBgColor =
    SECTION_COLORS[currentSectionIdx % SECTION_COLORS.length];
                    ...
                    <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
                      <span
                        style={{
                          ...
                          border: "2px solid #ccc",
                          marginRight: 8,
                          background: isSelected(qi, opt.id)
                            ? "#2563eb"
                            : "#fff",
                          borderColor: isSelected(qi, opt.id)
                            ? "#2563eb"
                            : "#ccc",
                        }}
                      />
          ...
          {currentSectionIdx < surveySections.length - 1 ? (
            <Button
              ...
              disabled={
                ...
                responses[currentSectionIdx].includes("")
              }
              sx={{
                flex: { xs: "1 1 100%", sm: "auto" },
                background: "#18a16e",
